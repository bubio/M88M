...
# Find the start of DrawSettings
start_idx = content.find('void UIManager::DrawSettings(PC8801::Config& cfg, PC88* pc88, CoreRunner* coreRunner) {')
if start_idx == -1:
    print("Could not find DrawSettings")
    exit(1)

# Find the end of DrawSettings (next function is DrawStatusBar)
end_idx = content.find('void UIManager::DrawStatusBar(DiskManager* diskmgr) {', start_idx)
if end_idx == -1:
    print("Could not find DrawStatusBar")
    exit(1)

draw_settings_new = """void UIManager::DrawSettings(PC8801::Config& cfg, PC88* pc88, CoreRunner* coreRunner) {
...
content = content[:start_idx] + draw_settings_new + "\n" + content[end_idx:]
